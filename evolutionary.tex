...
Nevertheless, in the SH game it can be easily shown that the inner
...
stable.
%Using an alternative definition of the ESS than that presented in definition 
%\ref{def:ess} will clarify the connection of asymptotically stable rest 
%points and an ESS.
%Let $(\epsilon e_i + (1-\epsilon) \vec{x})$ denote a population state invaded
%by some outsiders of share $\epsilon \in (0,1)$ with strategy $e_i$. A (mixed) strategy
%$\vec{x} \in \Delta$, likewise interpreted as population state, is said to be
%an ESS if the expected payoff of that strategy in the invaded population 
%state is higher than the expected payoff to the strategy of the invaders.
...
%An invasion barrier $\bar{\epsilon}$ is the maximal share of invaders 
