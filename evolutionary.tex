...
strategy $j$ observed, $F^j(x)$, and his
payoff $F^i(x)$.
...
$\bar{F}(\vec{x}) = x_1 \hat{F}^1(\vec{x}) + x_2 \hat{F}^2(\vec{x})$.
...
Contrary to the interpretation that agents follow a revision protocols 
...
However, in the deterministic case it is not clear what specifies the 
initial conditions. 
