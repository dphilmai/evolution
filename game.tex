...
\section{Coordination and network externality}
...
this direction, hence receiving only $b<c$.
Coordination on proprietary software leaves both of them with $d$, which 
satisfies, despite the success to coordinate, $a> \geq d>b$ as they have to
...
proprietary software. However, the students do not play the game against
one fellow student all the time, but are randomly assigned by their 
professor into team of two.
Additionally, I assume that they are not perfectly rational and
do not revaluate their software choice each time they have to do a project. 
For simplifaction, students in this model behave according to the 
...
only change it, if they get to know someone being more successfull with
...
never below the utility of coordination on proprietary software. Otherwise,
the structure of the stage game would differ fundamentally. So to say,
the network externality just adds to the fact that open source 
software is cheaper.
...
effect of the network externality $\frac{d^2e(x)}{dx^2} <0$, I will here
focus on the linear form $e(x) = \beta x$, with the externality parameter
$\beta \in \realnumb_+$. This restriction does not change the implication
of the model, but makes it less tedious to derive it.


Although the externality does not change the state in which
the whole population coordinated to one strategy, the inner fixed point
of the dynamic is changed. $\varphi(x)$ is still a polynominal but of 
degree $4$:
\begin{align}
...
evolving of this structure \parencite[46]{szabo_evolutionary_2007-1}.
For example \textcite{ohtsuki_simple_2006-1} found, that fewer connectivity 
leads to more cooperation, because "The fewer friends I have the more 
strongly my fate is bound to theirs" \parencite[1]{ohtsuki_simple_2006-1}.
